-- Drop and recreate get_nearby_jobs function to include job_category
DROP FUNCTION IF EXISTS public.get_nearby_jobs(double precision, double precision, double precision);

CREATE FUNCTION public.get_nearby_jobs(user_lat double precision, user_lng double precision, radius_km double precision DEFAULT 50)
RETURNS TABLE(
  id uuid, 
  employer_id uuid, 
  title text, 
