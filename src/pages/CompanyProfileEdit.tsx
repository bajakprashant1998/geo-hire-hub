...
const CompanyProfileEdit = () => {
  ...
  const autoSaveDocument = async (field: 'office_photo_url' | 'business_card_url', url: string) => {
    ...
    try {
      ...
      if (field === 'office_photo_url') {
        setOfficePhotoUrl(url);
      } else {
        setBusinessCardUrl(url);
      }

      // We don't need a success toast since DocumentUpload already shows one 
      // when the file successfully uploads to storage.

      // Recalculate completeness when documents are uploaded
      const { data: calcData } = await supabase
        .rpc('calculate_employer_profile_completeness', { p_employer_id: employerId });
